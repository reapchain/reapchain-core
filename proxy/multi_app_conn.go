...
// AppConns is the Reapchain's interface to the application that consists of
...
func (app *multiAppConn) OnStart() error {
	...
	// Kill Reapchain if the ABCI application crashes.
...
func (app *multiAppConn) killTMOnClientError() {
	killFn := func(conn string, err error, logger tmlog.Logger) {
		logger.Error(
			fmt.Sprintf("%s connection terminated. Did the application crash? Please restart reapchain", conn),
			"err", err)
		killErr := tmos.Kill()
		if killErr != nil {
