...
ReapchainCore light clients allow bandwidth & compute-constrained devices, such as
smartphones, low-power embedded chips, or other blockchains to efficiently
verify the consensus of a ReapchainCore blockchain. This forms the basis of safe
and efficient state synchronization for new network nodes and inter-blockchain
communication (where a light client of one ReapchainCore instance runs in another
...
NOTE: ReapchainCore provides a somewhat different (stronger) light client model
...
https://github.com/reapchain-core/spec/blob/master/spec/consensus/light-client/verification.md
for details.
...
ReapchainCore RPC exposes a lot of info, but a malicious node could return any
...
See
https://docs.reapchain-core.com/master/reapchain-core-core/light-client-protocol.html
for usage example.
Or see
https://github.com/reapchain-core/spec/tree/master/spec/consensus/light-client
for the full spec
*/
package light
