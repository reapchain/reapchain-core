...
Reapchain light clients allow bandwidth & compute-constrained devices, such as
smartphones, low-power embedded chips, or other blockchains to efficiently
verify the consensus of a Reapchain blockchain. This forms the basis of safe
and efficient state synchronization for new network nodes and inter-blockchain
communication (where a light client of one Reapchain instance runs in another
...
https://gitlab.reappay.net/reapchain/spec/blob/master/spec/consensus/light-client/verification.md
for details.
...
Reapchain RPC exposes a lot of info, but a malicious node could return any
...
See
https://docs.reapchain.com/master/reapchain-core/light-client-protocol.html
for usage example.
Or see
https://gitlab.reappay.net/reapchain/spec/tree/master/spec/consensus/light-client
for the full spec
*/
package light
