...
func (conR *Reactor) Receive(chID byte, src p2p.Peer, msgBytes []byte) {
	...
	switch chID {
	case QrnChannel:
		fmt.Println("stompesi-Receive-QrnChannel")
		if conR.WaitSync() {
	...
	case VoteChannel:
		// fmt.Println("stompesi-receive", string(debug.Stack()))
...
OUTER_LOOP:
	for {
		...
		if consensusRoundState.ConsensusRound.ConsensusStartBlockHeight+consensusRoundState.ConsensusRound.Peorid == consensusRoundState.Height {
			fmt.Println("gossipQrnRoutine",
				"consensusRoundState.ConsensusRound.ConsensusStartBlockHeight", consensusRoundState.ConsensusRound.ConsensusStartBlockHeight,
				"consensusRoundState.Height", consensusRoundState.Height,
				"peerRoundState.Height", peerRoundState.Height)
			// if consensusRoundState.Height == peerRoundState.Height {
			// 	if peerState.PickSendQrn(consensusRoundState.QrnSet) {
			// 		continue OUTER_LOOP
			// 	}
			// }
		}
		...
		// TODO: stompesi
		// if conR.conS.ConsensusRound.ConsensusStartBlockHeight == conR.conS.GetLastHeight() {
		// fmt.Println(conR.conS.GetLastHeight())
		// }

		// qrn := conR.conS.QrnSet.GetQrn(conR.conS.privValidatorPubKey)
		// if consensusRoundState.Height == peerRoundState.Height {
		// fmt.Println("stompesi-gossipQrnRoutine")
		//
		// }
...
func (conR *Reactor) peerStatsRoutine() {
	for {
		...
		case msg := <-conR.conS.statsMsgQueue:
			...
			//TODO: stompesi
			case *QrnMessage:
				fmt.Println("peerStatsRoutine", msg.PeerID)
			case *VoteMessage:
