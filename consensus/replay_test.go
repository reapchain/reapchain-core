...
func testHandshakeReplay(t *testing.T, config *cfg.Config, nBlocks int, mode uint, testValidatorsChange bool) {
	...
	// run the chain through state.ApplyBlock to build up the reapchain state
	...
	if nBlocks > 0 {
		...
		// use a throwaway reapchain state
...
func buildTMStateFromChain(
	...
	nBlocks int,
	mode uint) sm.State {
	// run the whole chain against this client to build up the reapchain state
...
func TestHandshakePanicsIfAppReturnsWrongAppHash(t *testing.T) {
	// 1. Initialize reapchain and commit 3 blocks with the following app hashes:
	//		- 0x01
	//		- 0x02
	//		- 0x03
	...
	// 2. Reapchain must panic if app returns wrong hash for the first block
	//		- RANDOM HASH
	//		- 0x02
	//		- 0x03
	{
	...
	// 3. Reapchain must panic if app returns wrong hash for the last block
	//		- 0x01
	//		- 0x02
	//		- RANDOM HASH
	{
