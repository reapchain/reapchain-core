...
//TODO: stompesi
func (peerState *PeerState) PickSendQrn(qrnSet *types.QrnSet) bool {
	if qrn, ok := peerState.PickQrnToSend(qrnSet); ok {
		fmt.Println("PickSendQrn")
		msg := &QrnMessage{qrn}
		if peerState.peer.Send(QrnChannel, MustEncode(msg)) {
			peerState.SetHasQrn(qrn)
			return true
		}
		return false
...
func (ps *PeerState) SetHasQrn(qrn *types.Qrn) {
	...
	peerStateQrnArray := ps.getQrnBitArray(qrn.Height)
	if peerStateQrnArray != nil {
		peerStateQrnArray.SetIndex(int(qrn.StandingMemberIndex), true)
	}
}

func (peerState *PeerState) getQrnBitArray(height int64) *bits.BitArray {
	if peerState.PRS.Height == height {
		return peerState.PRS.QrnBitArray
	}
	return nil
}

func (peerState *PeerState) PickQrnToSend(qrnSet *types.QrnSet) (qrn *types.Qrn, ok bool) {
	peerState.mtx.Lock()
	...
	// Lazily set data using 'votes'.
	if peerState.PRS.Height == height {
		if peerState.PRS.QrnBitArray == nil {
			peerState.PRS.QrnBitArray = bits.NewBitArray(size)
		}
	}

	if peerState.PRS.Step == cstypes.RoundStepNewHeight || peerState.PRS.Step == cstypes.RoundStepNewHeight {
		if index, ok := qrnSet.BitArray().Sub(peerState.PRS.QrnBitArray).PickRandom(); ok {
