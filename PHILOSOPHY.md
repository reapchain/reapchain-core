...
The design goals for ReapchainCore (and the SDK and related libraries) are:
...
ReapchainCore implementation will at least be an order of magnitude within the
range of what is theoretically possible.  The design philosophy of ReapchainCore,
and the choice of Go as implementation language, is designed to make ReapchainCore
...
as well as creating sufficiently performant software. ReapchainCore Core will
never be as fast as future implementations of the ReapchainCore Spec, because Go
...
Instead of optimizing ReapchainCore performance down to the metal, lets focus on
...
For example, mutexes are cheap enough for ReapchainCore's design goals when there
...
In order for ReapchainCore to remain relevant in the years to come, it is vital
for ReapchainCore to take advantage of multicore architectures.  Due to the nature
...
unavoidable for ReapchainCore development to require expertise in concurrency
...
aware of the debt and do not invest in the debt.  On the other hand, ReapchainCore
