...
func init() {
	...
	LightCmd.Flags().StringVarP(&primaryAddr, "primary", "p", "",
		"connect to a podc node at this address")
	LightCmd.Flags().StringVarP(&witnessAddrsJoined, "witnesses", "w", "",
		"podc nodes to cross-check the primary node, comma-separated")
	LightCmd.Flags().StringVar(&home, "home-dir", os.ExpandEnv(filepath.Join("$HOME", ".podc-light")),
