...
import (
	...
	lproxy "gitlab.reappay.net/reapchain/reapchain-core/light/proxy"
...
var LightCmd = &cobra.Command{
	Use:   "light [chainID]",
	Short: "Run a light client proxy server, verifying Reapchain rpc",
	Long: `Run a light client proxy server, verifying Reapchain rpc.

All calls that can be tracked back to a block header by a proof
will be verified before passing them back to the caller. Other than
that, it will present the same interface as a full Reapchain node.
...
func init() {
	...
	LightCmd.Flags().StringVarP(&witnessAddrsJoined, "witnesses", "w", "",
		"reapchain nodes to cross-check the primary node, comma-separated")
	LightCmd.Flags().StringVar(&home, "home-dir", os.ExpandEnv(filepath.Join("$HOME", ".reapchain-light")),
	...
	if primaryAddr == "" { // check to see if we can start from an existing state
		...
		if primaryAddr == "" {
			return errors.New("no primary address was provided nor found. Please provide a primary (using -p)." +
				" Run the command: reapchain light --help for more information")
		}
	} else {
