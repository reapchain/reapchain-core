...
// dumpNetInfo gets network information state dump from the Reapchain RPC and
...
// dumpConsensusState gets consensus state dump from the Reapchain RPC and
// writes it to file. It returns an error upon failure.
...
// copyWAL copies the Reapchain node's WAL file. It returns an error if the
...
// copyConfig copies the Reapchain node's config file. It returns an error if
