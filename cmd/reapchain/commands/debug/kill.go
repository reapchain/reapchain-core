...
var killCmd = &cobra.Command{
	...
	Short: "Kill a Reapchain process while aggregating and packaging debugging data",
	Long: `Kill a Reapchain process while also aggregating Reapchain process data
...
Example:
$ reapchain debug 34255 /path/to/tm-debug.zip`,
...
func killCmdHandler(cmd *cobra.Command, args []string) error {
	...
	tmpDir, err := ioutil.TempDir(os.TempDir(), "reapchain_debug_tmp")
	if err != nil {
	...
	logger.Info("killing Reapchain process")
...
// killProc attempts to kill the Reapchain process with a given PID with an
...
func killProc(pid uint64, dir string) error {
	// pipe STDERR output from tailing the Reapchain process to a file
	//
	...
	// kill the underlying Reapchain process and subsequent tailing process
	go func() {
		// Killing the Reapchain process with the '-ABRT|-6' signal will result in
		// a goroutine stacktrace.
		p, err := os.FindProcess(int(pid))
		if err != nil {
			fmt.Fprintf(os.Stderr, "failed to find PID to kill Reapchain process: %s", err)
		} else if err = p.Signal(syscall.SIGABRT); err != nil {
			fmt.Fprintf(os.Stderr, "failed to kill Reapchain process: %s", err)
		}

		// allow some time to allow the Reapchain process to be killed
		//
		...
		if err := cmd.Process.Kill(); err != nil {
			fmt.Fprintf(os.Stderr, "failed to kill Reapchain process output redirection: %s", err)
		}
	}()
