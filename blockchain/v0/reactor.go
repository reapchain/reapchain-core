...
// 	stateRequestsCh <-chan StateRequest
...
// 	stateRequestsCh := make(chan StateRequest, maxTotalRequesters)
...
// 	}
// 	pool := NewBlockPool(startHeight, requestsCh, stateRequestsCh, errorsCh)
...
// 		fmt.Println("StateResponse", state.QrnSet.Height)

// 		if err != nil {
// 			bcR.Logger.Error("State content is invalid", "err", err)
// 			return
// 		}
// 		bcR.pool.AddState(src.ID(), state, len(msgBytes))
...
// 		bcR.pool.SetStatePeerRange(src.ID(), msg.Base, msg.Height)
...
// 			height, numPending, numStatePending, lenRequesters, lenStateRequesters := bcR.pool.GetStatus()
...
// 			firstState, secondState := bcR.pool.PeekTwoStates()
...
// 				statePeerID := bcR.pool.RedoStateRequest(first.Height)
// 				statePeer := bcR.Switch.Peers().Get(statePeerID)
// 				if statePeer != nil {
// 					// NOTE: we've already removed the statePeer's request, but we
// 					// still need to clean up the rest.
// 					bcR.Switch.StopPeerForError(statePeer, fmt.Errorf("blockchainReactor validation error: %v", err))
// 				}
// 				statePeerID2 := bcR.pool.RedoStateRequest(second.Height)
// 				statePeer2 := bcR.Switch.Peers().Get(statePeerID2)
// 				if statePeer2 != nil && statePeer2 != statePeer {
// 					// NOTE: we've already removed the statePeer's request, but we
// 					// still need to clean up the rest.
// 					bcR.Switch.StopPeerForError(statePeer2, fmt.Errorf("blockchainReactor validation error: %v", err))
// 				}
// 				continue FOR_LOOP
// 			} else {
// 				bcR.pool.PopRequest()
// 				bcR.pool.PopStateRequest()
...
// 				fmt.Println("poolRoutine", firstState.QrnSet.Qrns[0].Value)
// 				fmt.Println("poolRoutine", firstState.QrnSet.Qrns[1].Value)
// 				fmt.Println("poolRoutine", firstState.QrnSet.Qrns[2].Value)
// 				fmt.Println("poolRoutine-NextQrnSet[0]", firstState.NextQrnSet.Qrns[0].Value)
// 				fmt.Println("poolRoutine-NextQrnSet[1]", firstState.NextQrnSet.Qrns[1].Value)
// 				fmt.Println("poolRoutine-NextQrnSet[2]", firstState.NextQrnSet.Qrns[2].Value)
// 				fmt.Println("poolRoutine", firstState.VrfSet.Vrfs[0].Value)
// 				fmt.Println("poolRoutine", firstState.VrfSet.Vrfs[1].Value)
