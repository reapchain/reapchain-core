...
// 	// state requests
// 	stateRequesters    map[int64]*bpRequester
// 	stateHeight        int64 // the lowest key in state requesters.
// 	statePeers         map[p2p.ID]*bpPeer
// 	maxStatePeerHeight int64 // the biggest reported height
// 	numStatePending    int32 // number of requests pending assignment or block response
...
// func NewBlockPool(start int64, requestsCh chan<- BlockRequest, stateRequestsCh chan<- StateRequest, errorsCh chan<- peerError) *BlockPool {
// 	bp := &BlockPool{
// 		peers:      make(map[p2p.ID]*bpPeer),
// 		statePeers: make(map[p2p.ID]*bpPeer),

// 		requesters: make(map[int64]*bpRequester),
// 		height:     start,
// 		numPending: 0,

// 		stateRequesters: make(map[int64]*bpRequester),
// 		stateHeight:     start,
...
// 		_, numPending, numStatePending, lenRequesters, lenStateRequesters := pool.GetStatus()
// 		switch {
...
// 		case numStatePending >= maxPendingRequests:
// 			// sleep for a bit.
// 			time.Sleep(requestIntervalMS * time.Millisecond)
// 			// check for timed out peers
// 			pool.removeStateTimedoutPeers()
// 		case lenStateRequesters >= maxTotalRequesters:
// 			// sleep for a bit.
// 			time.Sleep(requestIntervalMS * time.Millisecond)
// 			// check for timed out peers
// 			pool.removeStateTimedoutPeers()
...
// // requesters.
// func (pool *BlockPool) GetStatus() (height int64, numPending int32, numStatePending int32, lenRequesters int, lenStateRequesters int) {
// 	pool.mtx.Lock()
// 	defer pool.mtx.Unlock()

// 	return pool.height, atomic.LoadInt32(&pool.numPending), atomic.LoadInt32(&pool.numStatePending), len(pool.requesters), len(pool.stateRequesters)
// }
