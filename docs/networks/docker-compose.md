...
1. [Install reapchain-core](../introduction/install.md)
2. [Install docker](https://docs.docker.com/engine/installation/)
3. [Install docker-compose](https://docs.docker.com/compose/install/)

## Build

Build the `reapchain-core` binary and, optionally, the `reapchain-core/localnode`
docker image.
...
# (optionally) Build reapchain-core/localnode image
...
calling the `reapchain-core testnet` command.

The `./build` directory is mounted to the `/reapchain-core` mount point to attach
...
localnet-start: localnet-stop
  @if ! [ -f build/node0/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/reapchain-core:Z reapchain-core/localnode testnet --v 5 --n 3 --o . --populate-persistent-peers --starting-ip-address 192.167.10.2 ; fi
  docker-compose up
```
...
```yml
  node3: # bump by 1 for every node
    ...
    image: "reapchain-core/localnode"
    environment:
      - ID=3
...
To use your own ABCI applications with 4-node setup edit the [docker-compose.yaml](https://github.com/reapchain/reapchain-core/blob/v0.34.x/docker-compose.yml) file and add image to your ABCI application.
...
Override the [command](https://github.com/reapchain/reapchain-core/blob/v0.34.x/networks/local/localnode/Dockerfile#L12) in each node to connect to it's ABCI.
...
Similarly do for node1, node2 and node3 then [run testnet](https://github.com/reapchain/reapchain-core/blob/v0.34.x/docs/networks/docker-compose.md#run-a-testnet)

## Logging

Log is saved under the attached volume, in the `reapchain-core.log` file. If the
