...
`reapchain-core node --proxy_app=kvstore` on all droplets. Although we'll
...
```sh
ExecStart=/usr/bin/reapchain-core node --proxy_app=kvstore
```
...
to re-run `reapchain-core node` with the new flag, on all droplets. The
