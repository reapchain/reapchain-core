...
Reapchain Core. [Cosmos SDK](https://github.com/cosmos/cosmos-sdk) is written
this way. Please refer to [Writing a built-in Reapchain Core application in
...
it upon receiving SIGTERM or Ctrl-C. Reapchain Core will act as a client,
...
```bash
TMHOME="/tmp/example" reapchain node --proxy_app=unix://example.sock
