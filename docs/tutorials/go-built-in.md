...
Running your application inside the same process as ReapchainCore Core will give
you the best possible performance.

For other languages, your application have to communicate with ReapchainCore Core
...
When ReapchainCore Core has decided on the block, it's transfered to the
...
## 1.4 Starting an application and a ReapchainCore Core instance in the same process
...
func main() {
	...
	node, err := newReapchainCore(app, configFile)
	if err != nil {
...
func newReapchainCore(app abci.Application, configFile string) (*nm.Node, error) {
 // read config
 ...
 node, err := nm.NewNode(
  ...
  logger)
 if err != nil {
  return nil, fmt.Errorf("failed to create new ReapchainCore node: %w", err)
 }
...
Then we use it to create a ReapchainCore Core `Node` instance:
