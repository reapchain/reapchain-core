...
Amino has been a pain point of many users in the ecosystem. While Reapchain does not suffer greatly from the performance degradation introduced by amino, we are making an effort in moving the encoding format to a widely adopted format, [Protocol Buffers](https://developers.google.com/protocol-buffers). With this migration a new standard is needed for the encoding of keys. This will cause ecosystem wide breaking changes.

Currently amino encodes keys as `<PrefixBytes> <Length> <ByteArray>`.

## Decision

Previously Reapchain defined all the key types for use in Reapchain and the Cosmos-SDK. Going forward the Cosmos-SDK will define its own protobuf type for keys. This will allow Reapchain to only define the keys that are being used in the codebase (ed25519).
...
2. Move Reapchain types to protobuf, mainly the ones that are being encoded.
