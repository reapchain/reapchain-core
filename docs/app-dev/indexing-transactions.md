...
ReapchainCore allows you to index transactions and blocks and later query or
...
[ABCI](https://github.com/reapchain-core/spec/blob/master/spec/abci/abci.md#events)
documentation.
...
By default, ReapchainCore will index all transactions by their respective hashes
...
underlying ReapchainCore database. Using the `kv` indexer type allows you to query
for block and transaction events directly against ReapchainCore's RPC. However, the
...
searching is not enabled for the `psql` indexer type via ReapchainCore's RPC -- any
such query will fail.

Note, the SQL schema is stored in `state/indexer/sink/psql/schema.sql` and operators
must explicitly create the relations prior to starting ReapchainCore and enabling
...
The ReapchainCore tx and block event indexer indexes a few select reserved events
by default.
...
Applications are free to define which events to index. ReapchainCore does not
...
Check out [API docs](https://docs.reapchain-core.com/master/rpc/#/Info/tx_search)
...
Check out [API docs](https://docs.reapchain-core.com/master/rpc/#subscribe) for more information
...
Check out [API docs](https://docs.reapchain-core.com/master/rpc/#/Info/block_search)
for more information on query syntax and other options.
