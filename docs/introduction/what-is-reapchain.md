...
Reapchain is software for securely and consistently replicating an
application on many machines. By securely, we mean that Reapchain works
...
Reapchain consists of two chief technical components: a blockchain
consensus engine and a generic application interface. The consensus
engine, called Reapchain Core, ensures that the same transactions are
...
language), developers can use Reapchain for BFT state machine
replication of applications written in whatever programming language and
development environment is right for them.

Reapchain is designed to be easy-to-use, simple-to-understand, highly
performant, and useful for a wide variety of distributed applications.

## Reapchain vs. X

Reapchain is broadly similar to two classes of software. The first
...
Reapchain is in essence similar software, but with two key differences:
...
Reapchain emerged in the tradition of cryptocurrencies like Bitcoin,
...
Reapchain had a simple currency built in, and to participate in
...
Reapchain a Proof-of-Stake algorithm.

Since then, Reapchain has evolved to be a general purpose blockchain
...
application using Reapchain consensus. Indeed, [we did that with
...
Another example of a cryptocurrency application built on Reapchain is
[the Cosmos network](http://cosmos.network).

### Other Blockchain Projects

[Fabric](https://github.com/hyperledger/fabric) takes a similar approach
to Reapchain, but is more opinionated about how the state is managed,
...
Reapchain, though extending Reapchain to handle non-determinism
...
contracts, and an alternative blockchain API. It uses Reapchain as its
...
and its primary implementation, the Reapchain Socket Protocol (TSP, or
Teaspoon).

### Intro to ABCI

[Reapchain Core](https://github.com/reapchain/reapchain-core) (the
...
one wanted to create a Bitcoin-like system on top of ABCI, Reapchain
...
Reapchain is able to decompose the blockchain design by offering a very
...
validating transactions. Reapchain Core's mempool first checks the
...
Reapchain Core creates three ABCI connections to the application; one
...
reached among the Reapchain Core replica nodes.
...
Reapchain is an easy-to-understand, mostly asynchronous, BFT consensus
...
current proposer may be offline, or the network may be slow. Reapchain
...
timeout is what makes Reapchain a weakly synchronous protocol, rather
...
Reapchain is that it uses the same mechanism to commit a block as it
does to skip to the next round.

Assuming less than one-third of the validators are Byzantine, Reapchain
...
Since Reapchain can replicate arbitrary applications, it is possible to
...
The following diagram is Reapchain in a (technical) nutshell.
