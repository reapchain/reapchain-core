...
## reapchain debug kill

Reapchain comes with a `debug` sub-command that allows you to kill a live
Reapchain process while collecting useful information in a compressed archive.
...
```bash
reapchain debug kill <pid> </path/to/out.zip> --home=</path/to/app.d>
```
...
```bash
reapchain debug dump </path/to/out> --home=</path/to/app.d>
```
