...
## reapchain-core debug kill

ReapchainCore comes with a `debug` sub-command that allows you to kill a live
ReapchainCore process while collecting useful information in a compressed archive.
...
ReapchainCore process.

```bash
reapchain-core debug kill <pid> </path/to/out.zip> --home=</path/to/app.d>
```
...
```bash
reapchain-core debug dump </path/to/out> --home=</path/to/app.d>
```
