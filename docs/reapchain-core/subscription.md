...
ReapchainCore emits different events, which you can subscribe to via
...
Check out [API docs](https://docs.reapchain-core.com/master/rpc/) for
...
ReapchainCore receives from ABCI application (see [EndBlock
section](https://github.com/reapchain-core/spec/blob/master/spec/abci/abci.md#endblock) in
the ABCI spec).
...
```json
{
    ...
    "result": {
        ...
        "data": {
            "type": "reapchain-core/event/ValidatorSetUpdates",
            "value": {
              "validator_updates": [
                {
