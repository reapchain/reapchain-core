...
By default, Reapchain uses the `syndtr/goleveldb` package for its in-process
key-value database. If you want maximal performance, it may be best to install
the real C-implementation of LevelDB and compile Reapchain to use that using
`make build REAPCHAIN_BUILD_OPTIONS=cleveldb`. See the [install
instructions](../introduction/install.md) for details.

Reapchain keeps multiple distinct databases in the `$TMROOT/data`:
...
By default, Reapchain will only index txs by their hash and height, not by their DeliverTx
...
you're trying to debug Reapchain or asked to provide logs with debug
logging level, you can do so by running Reapchain with
`--log_level="*:debug"`.

## Write Ahead Logs (WAL)

Reapchain uses write ahead logs for the consensus (`cs.wal`) and the mempool
...
validator. Since Reapchain validators are expected to never sign a conflicting vote, the
...
The core of the Reapchain peer-to-peer system is `MConnection`. Each
...
elements (100 max). See the [RPC Documentation](https://docs.reapchain.com/master/rpc/)
...
If you ever have to debug Reapchain, the first thing you should probably do is
...
what's happening, consider using `reapchain debug kill` sub-command. This
...
## Monitoring Reapchain

Each Reapchain instance has a standard `/health` RPC endpoint, which responds
...
Reapchain also can report and serve Prometheus metrics. See
[Metrics](./metrics.md).

`reapchain debug dump` sub-command can be used to periodically dump useful
information into an archive. See [Debugging](../tools/debugging.md) for more
information.

## What happens when my app dies

You are supposed to run Reapchain under a [process
supervisor](https://en.wikipedia.org/wiki/Process_supervision) (like
systemd or runit). It will ensure Reapchain is always running (despite
possible errors).

Getting back to the original question, if your application dies,
Reapchain will panic. After a process supervisor restarts your
application, Reapchain should be able to reconnect successfully. The
...
**NOTE:** Make sure you have a backup of the Reapchain data directory.
...
- Admin error (e.g., directly modifying Reapchain data-directory contents)
...
Reapchain, replay will fail with panic.

Recovering from data corruption can be hard and time-consuming. Here are two approaches you can take:

1. Delete the WAL file and restart Reapchain. It will attempt to sync with other peers.
...
While for now, Reapchain stores all the history and it may require significant
...
Reapchain can be compiled for a wide range of operating systems thanks to Go
...
NOTE: if you are going to use Reapchain in a public domain, make sure
...
If you are going to use Reapchain in a private domain and you have a
...
After every block, Reapchain rechecks every transaction left in the
...
By default, Reapchain checks whenever a peer's address is routable before
