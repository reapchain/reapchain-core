...
By default, ReapchainCore uses the `syndtr/goleveldb` package for its in-process
key-value database. If you want maximal performance, it may be best to install
the real C-implementation of LevelDB and compile ReapchainCore to use that using
`make build REAPCHAIN_BUILD_OPTIONS=cleveldb`. See the [install
instructions](../introduction/install.md) for details.

ReapchainCore keeps multiple distinct databases in the `$TMROOT/data`:
...
By default, ReapchainCore will only index txs by their hash and height, not by their DeliverTx
...
you're trying to debug ReapchainCore or asked to provide logs with debug
logging level, you can do so by running ReapchainCore with
`--log_level="*:debug"`.

## Write Ahead Logs (WAL)

ReapchainCore uses write ahead logs for the consensus (`cs.wal`) and the mempool
...
validator. Since ReapchainCore validators are expected to never sign a conflicting vote, the
...
The core of the ReapchainCore peer-to-peer system is `MConnection`. Each
...
elements (100 max). See the [RPC Documentation](https://docs.reapchain-core.com/master/rpc/)
...
## Debugging ReapchainCore

If you ever have to debug ReapchainCore, the first thing you should probably do is
...
what's happening, consider using `reapchain-core debug kill` sub-command. This
...
[Github](https://github.com/reapchain/reapchain-core). Before opening an issue
however, be sure to check if there's [no existing
issue](https://github.com/reapchain/reapchain-core/issues) already.

## Monitoring ReapchainCore

Each ReapchainCore instance has a standard `/health` RPC endpoint, which responds
...
ReapchainCore also can report and serve Prometheus metrics. See
[Metrics](./metrics.md).

`reapchain-core debug dump` sub-command can be used to periodically dump useful
information into an archive. See [Debugging](../tools/debugging.md) for more
information.

## What happens when my app dies

You are supposed to run ReapchainCore under a [process
supervisor](https://en.wikipedia.org/wiki/Process_supervision) (like
systemd or runit). It will ensure ReapchainCore is always running (despite
possible errors).

Getting back to the original question, if your application dies,
ReapchainCore will panic. After a process supervisor restarts your
application, ReapchainCore should be able to reconnect successfully. The
...
**NOTE:** Make sure you have a backup of the ReapchainCore data directory.
...
- Admin error (e.g., directly modifying ReapchainCore data-directory contents)
...
ReapchainCore, replay will fail with panic.

Recovering from data corruption can be hard and time-consuming. Here are two approaches you can take:

1. Delete the WAL file and restart ReapchainCore. It will attempt to sync with other peers.
...
While for now, ReapchainCore stores all the history and it may require significant
disk space over time, we are planning to implement state syncing (See [this
issue](https://github.com/reapchain/reapchain-core/issues/828)). So, storing all
...
ReapchainCore can be compiled for a wide range of operating systems thanks to Go
...
NOTE: if you are going to use ReapchainCore in a public domain, make sure
...
If you are going to use ReapchainCore in a private domain and you have a
...
After every block, ReapchainCore rechecks every transaction left in the
...
By default, ReapchainCore checks whenever a peer's address is routable before
saving it to the address book. The address is considered as routable if the IP
is [valid and within allowed
ranges](https://github.com/reapchain/reapchain-core/blob/27bd1deabe4ba6a2d9b463b8f3e3f1e31b993e61/p2p/netaddress.go#L209).
