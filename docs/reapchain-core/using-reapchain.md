...
This is a guide to using the `reapchain` program from the command line.
It assumes only that you have the `reapchain` binary installed and have
some rudimentary idea of what Reapchain and ABCI are.

You can see the help menu with `reapchain --help`, and the version
number with `reapchain version`.

## Directory Root

The default directory for blockchain data is `~/.reapchain`. Override
...
```sh
reapchain testnet --help
```

### Genesis

The `genesis.json` file in `$TMHOME/config/` defines the initial
ReapchainCore state upon genesis of the blockchain ([see
definition](https://github.com/reapchain/reapchain-core/blob/master/types/genesis.go)).
...
- `initial_height`: Height at which Reapchain should begin at. If a blockchain is conducting a network upgrade, 
    starting from the stopped height brings uniqueness to previous heights. 
- `consensus_params` [spec](https://github.com/reapchain/spec/blob/master/spec/core/state.md#consensusparams)
    - `block`
...
- `app_hash`: The expected application hash (as returned by the
  ...
  not match, Reapchain will panic.
...
To run a Reapchain node, use:

```bash
reapchain node
```

By default, Reapchain will try to connect to an ABCI application on
`127.0.0.1:26658`. If you have the `kvstore` ABCI app installed, run it in
another window. If you don't, kill Reapchain and run an in-process version of
the `kvstore` app:
...
Reapchain supports in-process versions of the `counter`, `kvstore`, and `noop`
apps that ship as examples with `abci-cli`. It's easy to compile your app
in-process with Reapchain if it's written in Go. If your app is not written in
...
```bash
reapchain node --proxy_app=/var/run/abci.sock
```

You can find out what flags are supported by running `reapchain node --help`.

## Transactions

To send a transaction, use `curl` to make requests to the Reapchain RPC
server, for example:
...
> TIP: Find the RPC Documentation [here](https://docs.reapchain.com/master/rpc/)
...
Reapchain uses a `config.toml` for configuration. For details, see [the
config specification](./configuration.md).

Notable options include the socket address of the application
(`proxy_app`), the listening address of the Reapchain peer
...
While the default behavior of `reapchain` is still to create blocks
...
To configure Reapchain to not produce empty blocks unless there are
transactions or the app hash changes, run Reapchain with this
additional flag:

```sh
reapchain node --consensus.create_empty_blocks=false
```
...
transaction. When a transaction is sent to a Reapchain node, it will
...
write-ahead-log](../reapchain-core/running-in-production.md#mempool-wal)

## Reapchain Networks

When `reapchain init` is run, both a `genesis.json` and
`priv_validator_key.json` are created in `~/.reapchain/config`. The
...
`priv_validator_key.json`, so a Reapchain node started with the default
...
```sh
reapchain node --p2p.seeds "f9baeaa15fedf5e1ef7448dd60f46c01f1a9e9c4@1.2.3.4:26656,0491d373a8e0fcf1023aaf18c51d6a1d0d4f31bd@5.6.7.8:26656"
```
...
If you want Reapchain to connect to specific set of addresses and
...
stopping Reapchain core instance.

```sh
reapchain node --p2p.persistent_peers "429fcf25974313b95673f58d77eacdd434402665@10.11.12.13:26656,96663a3dd0d7b9d17d4c8211b191af259621c693@10.11.12.14:26656"
...
to `~/.reapchain/config` on the new machine and start the node,
...
Update the `genesis.json` in `~/.reapchain/config`. Copy the genesis
file and the new `priv_validator_key.json` to the `~/.reapchain/config` on
a new machine.

Now run `reapchain node` on both machines, and use either
...
To make a Reapchain network that can tolerate one of the validators
...
`addr_book_strict=false` in the `config.toml`, otherwise Reapchain's p2p
library will deny making connections to peers with the same IP address.

### Upgrading

See the
[UPGRADING.md](https://github.com/reapchain/reapchain-core/blob/master/UPGRADING.md)
guide. You may need to reset your chain between major breaking releases.
Although, we expect Reapchain to have fewer breaking releases in the future
