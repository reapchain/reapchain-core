...
applications. ReapchainCore provides unique speed and security properties for
...
[merkle proofs](https://github.com/reapchain-core/spec/blob/953523c3cb99fdb8c8f7a2d21e3a99094279e9de/spec/blockchain/encoding.md#iavl-tree).

## Properties

- You get the full collateralized security benefits of ReapchainCore; no
  need to wait for confirmations.
- You get the full speed benefits of ReapchainCore; transactions
  commit instantly.
...
ReapchainCore comes with a built-in `reapchain-core light` command, which can be used
to run a light client proxy server, verifying ReapchainCore RPC. All calls that
...
interface as a full ReapchainCore node.

You can start the light client proxy server by running `reapchain-core light <chainID>`,
...
```bash
$ reapchain-core light supernova -p tcp://233.123.0.140:26657 \
...
For additional options, run `reapchain-core light --help`.
