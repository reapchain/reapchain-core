...
applications. Reapchain provides unique speed and security properties for
...
[merkle proofs](https://github.com/reapchain/spec/blob/953523c3cb99fdb8c8f7a2d21e3a99094279e9de/spec/blockchain/encoding.md#iavl-tree).

## Properties

- You get the full collateralized security benefits of Reapchain; no
  need to wait for confirmations.
- You get the full speed benefits of Reapchain; transactions
  commit instantly.
...
Reapchain comes with a built-in `reapchain light` command, which can be used
to run a light client proxy server, verifying Reapchain RPC. All calls that
...
interface as a full Reapchain node.

You can start the light client proxy server by running `reapchain light <chainID>`,
...
```bash
$ reapchain light supernova -p tcp://233.123.0.140:26657 \
...
For additional options, run `reapchain light --help`.
