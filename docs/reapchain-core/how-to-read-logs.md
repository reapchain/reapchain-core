...
Then ReapchainCore Core and the application perform a handshake.
...
I[10-04|13:54:27.368] Completed ABCI Handshake - ReapchainCore and App are synced module=consensus appHeight=90 appHash=E0FBAFBF6FCED8B9786DDFEB1A0D4FA2501BADAD
```
...
I[10-04|13:54:30.387] Starting WAL                                 module=consensus wal=/home/vagrant/.reapchain-core/data/cs.wal/wal impl=WAL
I[10-04|13:54:30.388] Starting TimeoutTicker                       module=consensus impl=TimeoutTicker
```

Notice the second row where ReapchainCore Core reports that "This node is a
...
Algorithm](https://github.com/reapchain-core/spec/blob/master/spec/consensus/consensus.md).
...
Here is the list of modules you may encounter in ReapchainCore's log and a
little overview what they do.

- `abci-client` As mentioned in [Application Development Guide](../app-dev/app-development.md), ReapchainCore acts as an ABCI
  client with respect to the application and maintains 3 connections:
  mempool, consensus and query. The code used by ReapchainCore Core can
  be found [here](https://github.com/reapchain/reapchain-core/blob/v0.34.x/abci/client).
- `blockchain` Provides storage, pool (a group of peers), and reactor
  for both storing and exchanging blocks between peers.
- `consensus` The heart of ReapchainCore core, which is the
...
- `events` Simple event notification system. The list of events can be
  found
  [here](https://github.com/reapchain/reapchain-core/blob/v0.34.x/types/events.go).
...
- `p2p` Provides an abstraction around peer-to-peer communication. For
  ...
  [README](https://github.com/reapchain/reapchain-core/blob/v0.34.x/p2p/README.md).
- `rpc` [ReapchainCore's RPC](./rpc.md).
- `rpc-server` RPC server. For implementation details, please read the
  [doc.go](https://github.com/reapchain/reapchain-core/blob/v0.34.x/rpc/jsonrpc/doc.go).
