...
Reapchain Core can be configured via a TOML file in
...
The default configuration file create by `reapchain init` has all
the parameters set with their default values. It will look something
like the file below, however, double check by inspecting the
`config.toml` created with your version of `reapchain` installed:
...
# "$HOME/.reapchain" by default, but could be changed via $TMHOME env variable
# or --home cmd flag.
...
# Migth be either absolute path or path related to reapchain's config directory.
...
Note after the block H, Reapchain creates something we call a "proof block"
...
default (`0`), Reapchain will be creating empty blocks even in the absence of
...
Reapchain will only create blocks if there are transactions, or after waiting
