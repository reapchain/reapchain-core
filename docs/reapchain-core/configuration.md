...
ReapchainCore Core can be configured via a TOML file in
...
# Migth be either absolute path or path related to reapchain-core's config directory.
...
Note after the block H, ReapchainCore creates something we call a "proof block"
...
default (`0`), ReapchainCore will be creating empty blocks even in the absence of
...
ReapchainCore will only create blocks if there are transactions, or after waiting
