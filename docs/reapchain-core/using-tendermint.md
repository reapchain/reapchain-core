...
This is a guide to using the `reapchain-core` program from the command line.
It assumes only that you have the `reapchain-core` binary installed and have
some rudimentary idea of what ReapchainCore and ABCI are.

You can see the help menu with `reapchain-core --help`, and the version
number with `reapchain-core version`.
...
```sh
reapchain-core testnet --help
```

### Genesis

The `genesis.json` file in `$TMHOME/config/` defines the initial
ReapchainCoreCore state upon genesis of the blockchain ([see
...
- `initial_height`: Height at which ReapchainCore should begin at. If a blockchain is conducting a network upgrade, 
    starting from the stopped height brings uniqueness to previous heights. 
- `consensus_params` [spec](https://github.com/reapchain-core/spec/blob/master/spec/core/state.md#consensusparams)
    - `block`
...
- `app_hash`: The expected application hash (as returned by the
  ...
  not match, ReapchainCore will panic.
...
another window. If you don't, kill ReapchainCore and run an in-process version of
the `kvstore` app:
...
ReapchainCore supports in-process versions of the `counter`, `kvstore`, and `noop`
apps that ship as examples with `abci-cli`. It's easy to compile your app
in-process with ReapchainCore if it's written in Go. If your app is not written in
...
To send a transaction, use `curl` to make requests to the ReapchainCore RPC
server, for example:
...
> TIP: Find the RPC Documentation [here](https://docs.reapchain-core.com/master/rpc/)
...
ReapchainCore uses a `config.toml` for configuration. For details, see [the
config specification](./configuration.md).

Notable options include the socket address of the application
(`proxy_app`), the listening address of the ReapchainCore peer
...
While the default behavior of `reapchain-core` is still to create blocks
...
To configure ReapchainCore to not produce empty blocks unless there are
transactions or the app hash changes, run ReapchainCore with this
additional flag:
...
transaction. When a transaction is sent to a ReapchainCore node, it will
...
write-ahead-log](../reapchain-core-core/running-in-production.md#mempool-wal)
...
`priv_validator_key.json`, so a ReapchainCore node started with the default
...
If you want ReapchainCore to connect to specific set of addresses and
...
To make a ReapchainCore network that can tolerate one of the validators
...
`addr_book_strict=false` in the `config.toml`, otherwise ReapchainCore's p2p
library will deny making connections to peers with the same IP address.

### Upgrading

See the
[UPGRADING.md](https://github.com/reapchain/reapchain-core/blob/v0.34.x/UPGRADING.md)
guide. You may need to reset your chain between major breaking releases.
Although, we expect ReapchainCore to have fewer breaking releases in the future
