...
**Upgrading to Reapchain 0.34 requires a blockchain restart.**
This release is not compatible with previous blockchains due to changes to
the encoding format (see "Protocol Buffers," below) and the block header (see "Blockchain Protocol").

Note also that Reapchain 0.34 also requires Go 1.15 or higher. 
...
* New ABCI methods (`ListSnapshots`, `LoadSnapshotChunk`, `OfferSnapshot`, and `ApplySnapshotChunk`)
  ...
  Read [the spec](https://docs.reapchain.com/master/spec/abci/apps.html#state-sync)
...
* The [`PublicKey` type](https://gitlab.reappay.net/reapchain/reapchain-core/blob/master/proto/reapchain/crypto/keys.proto#L13-L15)
...
Reapchain now relies on the application to tell it which transactions to index. This means that
in the `config.toml`, generated by Reapchain, there is no longer a way to specify which
...
Applications can notify Reapchain to index a specific transaction by setting
...
Reapchain 0.34 replaces Amino with Protocol Buffers for encoding.
This migration is extensive and results in a number of changes, however,
Reapchain only uses the types generated from Protocol Buffers for disk and
wire serialization.
**This means that these changes should not affect you as a Reapchain user.**

However, Reapchain users and contributors may note the following changes:
...
For more on the Protobuf changes, please see our [blog post on this migration](https://medium.com/reapchain/reapchain-0-34-protocol-buffers-and-you-8c40558939ae).

### Consensus Parameters

Reapchain 0.34 includes new and updated consensus parameters.
...
* The multisig functionality that was previously in Reapchain now has
...
The `reapchain lite` command has been renamed to `reapchain light` and has a slightly different API.
See [the docs](https://docs.reapchain.com/master/reapchain-core/light-client-protocol.html#http-proxy) for details.

### Light Client

We have a new, rewritten light client! You can
[read more](https://medium.com/reapchain/everything-you-need-to-know-about-the-reapchain-light-client-f80d03856f98)
...
* The `state` package now has a `Store` interface. All functions in
  [state/store.go](https://gitlab.reappay.net/reapchain/reapchain-core/blob/56911ee35298191c95ef1c7d3d5ec508237aaff4/state/store.go#L42-L42)
...
Version is now set through Go linker flags `ld_flags`. Applications that are using reapchain as a library should set this at compile time. 

Example:

```sh
go install -mod=readonly -ldflags "-X gitlab.reappay.net/reapchain/reapchain-core/version.TMCoreSemVer=$(go list -m gitlab.reappay.net/reapchain/reapchain-core | sed  's/ /\@/g') -s -w " -trimpath ./cmd
```
...
ResponseCommit added a field for block retention. The application can provide information to Reapchain on how to prune blocks.
...
See [\#63](https://gitlab.reappay.net/reapchain/spec/pull/63) for the complete spec
change.
...
[here](https://gitlab.reappay.net/reapchain/reapchain-core/pull/3668).

### Config Changes

You will need to generate a new config if you have used a prior version of reapchain.

Tags have been entirely renamed throughout the codebase to events and there
keys are called
[compositeKeys](https://gitlab.reappay.net/reapchain/reapchain-core/blob/6d05c531f7efef6f0619155cf10ae8557dd7832f/docs/app-dev/indexing-transactions.md).
...
* `/block_results` response format updated [see RPC docs for details](https://docs.reapchain.com/master/rpc/#/Info/block_results)
...
For further documentation on `Events`, see the [docs](https://gitlab.reappay.net/reapchain/reapchain-core/blob/60827f75623b92eff132dc0eff5b49d2025c591e/docs/spec/abci/abci.md#events).
...
<https://reapchain.com/docs/introduction/install.html#compile-with-cleveldb-support>)
...
If your WS client is not consuming events fast enough, Reapchain can terminate the subscription.
...
```json
{
  ...
  "error": {
    "code": -32000,
    "msg": "Server error",
    "data": "subscription was cancelled (reason: client is not pulling messages fast enough)" // or "subscription was cancelled (reason: Reapchain exited)"
  }
}
...
* [Merkle tree](https://gitlab.reappay.net/reapchain/spec/blob/master/spec/blockchain/encoding.md#merkle-trees)
* [ConsensusParams](https://gitlab.reappay.net/reapchain/spec/blob/master/spec/blockchain/state.md#consensusparams)

There was also a small change to field ordering in the vote struct. Any
implementations of an out-of-process validator (like a Key-Management Server)
will need to be updated. For specific details:

* [Vote](https://gitlab.reappay.net/reapchain/spec/blob/master/spec/consensus/signing.md#votes)

Finally, the proposer selection algorithm continues to evolve. See the
[work-in-progress
specification](https://gitlab.reappay.net/reapchain/reapchain-core/pull/3140).
...
Reapchain. This is different from how Unix sockets used to work, where
Reapchain dialed the external process.
...
See [#3105](https://gitlab.reappay.net/reapchain/reapchain-core/issues/3105).
Note the Unix socket has neither encryption nor authentication, but will
add a shared-secret in [#3099](https://gitlab.reappay.net/reapchain/reapchain-core/issues/3099).
...
consolidated under the reapchain semantic version (ie. `node_info.version`) and
...
* [ADR-026](https://gitlab.reappay.net/reapchain/reapchain-core/blob/30519e8361c19f4bf320ef4d26288ebc621ad725/docs/architecture/adr-026-general-merkle-proof.md)
* [Relevant ABCI
  documentation](https://gitlab.reappay.net/reapchain/reapchain-core/blob/30519e8361c19f4bf320ef4d26288ebc621ad725/docs/spec/abci/apps.md#query-proofs)
* [Description of
  keys](https://gitlab.reappay.net/reapchain/reapchain-core/blob/30519e8361c19f4bf320ef4d26288ebc621ad725/crypto/merkle/proof_key_path.go#L14)
...
[here](https://gitlab.reappay.net/reapchain/reapchain-core/blob/30519e8361c19f4bf320ef4d26288ebc621ad725/cmd/reapchain/commands/run_node.go#L60).
