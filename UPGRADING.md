# Upgrading ReapchainCore Core

This guide provides instructions for upgrading to specific versions of ReapchainCore Core.
...
**Upgrading to ReapchainCore 0.34 requires a blockchain restart.**
This release is not compatible with previous blockchains due to changes to
the encoding format (see "Protocol Buffers," below) and the block header (see "Blockchain Protocol").

Note also that ReapchainCore 0.34 also requires Go 1.15 or higher.
...
ReapchainCore now relies on the application to tell it which transactions to index. This means that
in the `config.toml`, generated by ReapchainCore, there is no longer a way to specify which
...
Applications can notify ReapchainCore to index a specific transaction by setting
...
ReapchainCore 0.34 replaces Amino with Protocol Buffers for encoding.
This migration is extensive and results in a number of changes, however,
ReapchainCore only uses the types generated from Protocol Buffers for disk and
wire serialization.
**This means that these changes should not affect you as a ReapchainCore user.**

However, ReapchainCore users and contributors may note the following changes:
...
ReapchainCore 0.34 includes new and updated consensus parameters.
...
* The multisig functionality that was previously in ReapchainCore now has
...
ResponseCommit added a field for block retention. The application can provide information to ReapchainCore on how to prune blocks.
...
If your WS client is not consuming events fast enough, ReapchainCore can terminate the subscription.
...
```json
{
  ...
  "error": {
    "code": -32000,
    "msg": "Server error",
    "data": "subscription was cancelled (reason: client is not pulling messages fast enough)" // or "subscription was cancelled (reason: ReapchainCore exited)"
  }
}
...
Any implementations of ReapchainCore blockchain verification, including lite clients,
...
ReapchainCore. This is different from how Unix sockets used to work, where
ReapchainCore dialed the external process.
