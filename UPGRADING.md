...
Refactor](https://github.com/reapchain/reapchain-core/blob/master/docs/architecture/adr-067-mempool-refactor.md).

## v0.34.0

**Upgrading to Reapchain Core 0.34 requires a blockchain restart.**
This release is not compatible with previous blockchains due to changes to
the encoding format (see "Protocol Buffers," below) and the block header (see "Blockchain Protocol").

Note also that Reapchain Core 0.34 also requires Go 1.15 or higher.
...
* New ABCI methods (`ListSnapshots`, `LoadSnapshotChunk`, `OfferSnapshot`, and `ApplySnapshotChunk`)
  ...
  Read [the spec](https://docs.reapchain-core.com/master/spec/abci/apps.html#state-sync)
...
* The [`PublicKey` type](https://github.com/reapchain/reapchain-core/blob/master/proto/podc/crypto/keys.proto#L13-L15)
  ...
  Note that since Reapchain Core only supports ed25519 validator keys, there's only one
...
Reapchain Core now relies on the application to tell it which transactions to index. This means that
in the `config.toml`, generated by Reapchain Core, there is no longer a way to specify which
...
Applications can notify Reapchain Core to index a specific transaction by setting
...
Reapchain Core 0.34 replaces Amino with Protocol Buffers for encoding.
This migration is extensive and results in a number of changes, however,
Reapchain Core only uses the types generated from Protocol Buffers for disk and
wire serialization.
**This means that these changes should not affect you as a Reapchain Core user.**

However, Reapchain Core users and contributors may note the following changes:
...
For more on the Protobuf changes, please see our [blog post on this migration](https://medium.com/reapchain/reapchain-core-0-34-protocol-buffers-and-you-8c40558939ae).

### Consensus Parameters

Reapchain Core 0.34 includes new and updated consensus parameters.
...
* The multisig functionality that was previously in Reapchain Core now has
...
The `reapchain-core lite` command has been renamed to `reapchain-core light` and has a slightly different API.
See [the docs](https://docs.reapchain-core.com/master/reapchain-core-core/light-client-protocol.html#http-proxy) for details.

### Light Client

We have a new, rewritten light client! You can
[read more](https://medium.com/reapchain-core/everything-you-need-to-know-about-the-reapchain-core-light-client-f80d03856f98)
...
* The `state` package now has a `Store` interface. All functions in
  [state/store.go](https://github.com/reapchain/reapchain-core/blob/56911ee35298191c95ef1c7d3d5ec508237aaff4/state/store.go#L42-L42)
...
Version is now set through Go linker flags `ld_flags`. Applications that are using reapchain-core as a library should set this at compile time.

Example:

```sh
go install -mod=readonly -ldflags "-X github.com/reapchain/reapchain-core/version.TMCoreSemVer=$(go list -m github.com/reapchain/reapchain-core | sed  's/ /\@/g') -s -w " -trimpath ./cmd
```
...
ResponseCommit added a field for block retention. The application can provide information to Reapchain Core on how to prune blocks.
...
See [\#63](https://github.com/reapchain-core/spec/pull/63) for the complete spec
change.
...
[here](https://github.com/reapchain/reapchain-core/pull/3668).

### Config Changes

You will need to generate a new config if you have used a prior version of reapchain-core.

Tags have been entirely renamed throughout the codebase to events and there
keys are called
[compositeKeys](https://github.com/reapchain/reapchain-core/blob/6d05c531f7efef6f0619155cf10ae8557dd7832f/docs/app-dev/indexing-transactions.md).
...
* `/block_results` response format updated [see RPC docs for details](https://docs.reapchain-core.com/master/rpc/#/Info/block_results)
...
For further documentation on `Events`, see the [docs](https://github.com/reapchain/reapchain-core/blob/60827f75623b92eff132dc0eff5b49d2025c591e/docs/spec/abci/abci.md#events).
...
Use `cleveldb` tag instead of `gcc` to compile Reapchain Core with CLevelDB or
use `make build_c` / `make install_c` (full instructions can be found at
<https://reapchain-core.com/docs/introduction/install.html#compile-with-cleveldb-support>)
...
If your WS client is not consuming events fast enough, Reapchain Core can terminate the subscription.
...
```json
{
  ...
  "error": {
    "code": -32000,
    "msg": "Server error",
    "data": "subscription was cancelled (reason: client is not pulling messages fast enough)" // or "subscription was cancelled (reason: Reapchain Core exited)"
  }
}
...
Any implementations of Reapchain Core blockchain verification, including lite clients,
will need to be updated. For specific details:

* [Merkle tree](https://github.com/reapchain-core/spec/blob/master/spec/blockchain/encoding.md#merkle-trees)
* [ConsensusParams](https://github.com/reapchain-core/spec/blob/master/spec/blockchain/state.md#consensusparams)

There was also a small change to field ordering in the vote struct. Any
implementations of an out-of-process validator (like a Key-Management Server)
will need to be updated. For specific details:

* [Vote](https://github.com/reapchain-core/spec/blob/master/spec/consensus/signing.md#votes)

Finally, the proposer selection algorithm continues to evolve. See the
[work-in-progress
specification](https://github.com/reapchain/reapchain-core/pull/3140).
...
Reapchain Core. This is different from how Unix sockets used to work, where
Reapchain Core dialed the external process.
...
See [#3105](https://github.com/reapchain/reapchain-core/issues/3105).
Note the Unix socket has neither encryption nor authentication, but will
add a shared-secret in [#3099](https://github.com/reapchain/reapchain-core/issues/3099).
...
consolidated under the reapchain-core semantic version (ie. `node_info.version`) and
...
* [ADR-026](https://github.com/reapchain/reapchain-core/blob/30519e8361c19f4bf320ef4d26288ebc621ad725/docs/architecture/adr-026-general-merkle-proof.md)
* [Relevant ABCI
  documentation](https://github.com/reapchain/reapchain-core/blob/30519e8361c19f4bf320ef4d26288ebc621ad725/docs/spec/abci/apps.md#query-proofs)
* [Description of
  keys](https://github.com/reapchain/reapchain-core/blob/30519e8361c19f4bf320ef4d26288ebc621ad725/crypto/merkle/proof_key_path.go#L14)
