...
.. figure:: assets/t_plus_k.png
   :alt: Reapchain plus Kubernetes

   Reapchain plus Kubernetes
...
::
    ...
    git clone https://github.com/reapchain/tools.git && cd tools/mintnet-kubernetes/examples/basecoin && make create
...
then query the Reapchain app logs from the first pod:

``kubectl logs -c tm -f tm-0``

finally, use our `Rest API <https://reapchain.com/docs/reapchain-core/rpc.html>`__ to fetch the status of the second pod's Reapchain app.
...
::
    ...
    curl -Lo app.yaml https://github.com/reapchain/tools/raw/master/mintnet-kubernetes/app.template.yaml
...
Reapchain app logs from the first pod:
...
Status of the second pod's Reapchain app:
...
keys, it forms a genesis file and starts the Reapchain process.
