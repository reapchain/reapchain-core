...
func Setup(testnet *e2e.Testnet) error {
	...
	for _, node := range testnet.Nodes {
		...
		// Set up a dummy validator. Reapchain requires a file PV even when not used, so we
...
func MakeGenesis(testnet *e2e.Testnet) (types.GenesisDoc, error) {
	...
	// The validator set will be sorted internally by Reapchain ranked by power,
...
// MakeConfig generates a Reapchain config for a node.
func MakeConfig(node *e2e.Node) (*config.Config, error) {
	...
	// Reapchain errors if it does not have a privval key set up, regardless of whether
