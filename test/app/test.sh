...
function kvstore_over_socket_reorder(){
    rm -rf $TMHOME
    podc init
    echo "Starting kvstore_over_socket_reorder (ie. start podc first)"
    podc node > podc.log &
    pid_podc=$!
    sleep 2
    abci-cli kvstore > /dev/null &
...
function counter_over_grpc_grpc() {
    ...
    sleep 1
    GRPC_PORT=36656
    podc node --abci grpc --rpc.grpc_laddr tcp://localhost:$GRPC_PORT > podc.log &
    pid_podc=$!
    sleep 5
