...
export TMHOME=$HOME/.reapchain_app
...
function kvstore_over_socket(){
    ...
    kill -9 $pid_kvstore $pid_reapchain
}

# start reapchain first
function kvstore_over_socket_reorder(){
    rm -rf $TMHOME
    reapchain init
    echo "Starting kvstore_over_socket_reorder (ie. start reapchain first)"
    reapchain node > reapchain.log &
    pid_reapchain=$!
    sleep 2
    abci-cli kvstore > /dev/null &
...
function counter_over_socket() {
    ...
    kill -9 $pid_counter $pid_reapchain
}
...
function counter_over_grpc() {
    ...
    pid_counter=$!
    reapchain node --abci grpc > reapchain.log &
    pid_reapchain=$!
    sleep 5
...
function counter_over_grpc_grpc() {
    ...
    sleep 1
    GRPC_PORT=36656
    reapchain node --abci grpc --rpc.grpc_laddr tcp://localhost:$GRPC_PORT > reapchain.log &
    pid_reapchain=$!
    sleep 5
