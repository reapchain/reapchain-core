...
to a reapchain node, as well as higher-level functionality.

The main implementation for production code is client.HTTP, which
connects via http to the jsonrpc interface of the reapchain node.
...
type SignClient interface {
	...
	Qns(ctx context.Context, height *int64) (*ctypes.ResultQns, error)
...
// string. see reapchain/types/events.go
