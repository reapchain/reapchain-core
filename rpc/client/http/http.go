...
/*
HTTP is a Client implementation that communicates with a Reapchain node over
JSON RPC and WebSockets.

This is the main implementation you probably want to use in production code.
There are other implementations when calling the Reapchain node in-process
(Local), or when you want to mock out the server for test code (mock).

You can subscribe for any event published by Reapchain using Subscribe method.
Note delivery is best-effort. If you don't read events fast enough or network is
slow, Reapchain might cancel the subscription. The client will attempt to
...
func (w *WSEvents) Subscribe(ctx context.Context, subscriber, query string,
	...
	w.mtx.Lock()
	// subscriber param is ignored because Reapchain will override it with
	// remote IP anyway.
	...
	for {
		...
		case resp, ok := <-w.ws.ResponsesCh:
			...
			if resp.Error != nil {
				...
				// client) reached or Reapchain exited.
				// We can ignore ErrAlreadySubscribed, but need to retry in other
				// cases.
				if !isErrAlreadySubscribed(resp.Error) {
					// Resubscribe after 1 second to give Reapchain time to restart (if
					// crashed).
