...
func createConfig() *cfg.Config {
	...
	c.RPC.CORSAllowedOrigins = []string{"https://reapchain.com/"}
...
// StartReapchain starts a test reapchain server in a go routine and returns when it is initialized
func StartReapchain(app abci.Application, opts ...func(*Options)) *nm.Node {
	nodeOpts := defaultOptions
	for _, opt := range opts {
		opt(&nodeOpts)
	}
	node := NewReapchain(app, &nodeOpts)
	err := node.Start()
	if err != nil {
		panic(err)
	}
	...
	if !nodeOpts.suppressStdout {
		fmt.Println("Reapchain running!")
	}

	return node
}

// StopReapchain stops a test reapchain server, waits until it's stopped and
// cleans up test/config files.
func StopReapchain(node *nm.Node) {
...
// NewReapchain creates a new reapchain server and sleeps forever
func NewReapchain(app abci.Application, opts *Options) *nm.Node {
	// Create & start node
...
// SuppressStdout is an option that tries to make sure the RPC test Reapchain
